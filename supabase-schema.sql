...
CREATE TABLE IF NOT EXISTS public.orders (
    ...
    order_number TEXT UNIQUE NOT NULL, -- 格式: ORD-YYYYMMDD-XXXXX
    ...
    -- 配送信息
    shipping_address JSONB NOT NULL, -- {name, phone, address, city, province, postal_code, country}
    ...
    payment_intent_id TEXT, -- 用于第三方支付
...
CREATE TABLE IF NOT EXISTS public.order_items (
    ...
    product_snapshot JSONB NOT NULL, -- 保存下单时的产品快照 {name, image_url, sku}
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 创建购物车表 (用于登录用户的购物车持久化)
...
CREATE TABLE IF NOT EXISTS public.reviews (
    ...
    content TEXT,
    images JSONB, -- 评论图片
    is_verified BOOLEAN DEFAULT false, -- 是否为已购买用户
    helpful_count INTEGER DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(product_id, user_id) -- 每个用户对每个产品只能评论一次
);
...
CREATE TABLE IF NOT EXISTS public.product_variants (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    product_id UUID REFERENCES public.products(id) ON DELETE CASCADE,
    name TEXT NOT NULL, -- 变体名称，如 "红色-L码"
    sku TEXT UNIQUE,
    price DECIMAL(10,2),
    compare_price DECIMAL(10,2),
    weight DECIMAL(8,2),
...
CREATE TABLE IF NOT EXISTS public.inventory_logs (
    ...
    quantity INTEGER NOT NULL,
    reason TEXT,
    reference_id UUID, -- 关联的订单或其他记录ID
...
CREATE INDEX IF EXISTS idx_orders_created_at ON public.orders(created_at);
...
FROM public.users u
LEFT JOIN public.orders o ON u.id = o.user_id AND o.status NOT IN ('cancelled', 'refunded')
...
BEGIN
    ...
    -- 生成订单号: ORD-YYYYMMDD-XXXXX (5位数字，不足补零)
